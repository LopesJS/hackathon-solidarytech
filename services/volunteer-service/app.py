...
# FIX: lê AWS_ENDPOINT_URL do ambiente.
# - Em desenvolvimento local: aponta para o LocalStack (ex: http://localstack:4566)
# - Em produção na AWS: variável não definida → boto3 conecta nos endpoints reais da AWS
...
try:
    dynamodb = boto3.resource(
        ...
        endpoint_url=AWS_ENDPOINT_URL  # None em produção, URL do LocalStack em dev
    )
    table = dynamodb.Table(DYNAMODB_TABLE)
    log.info(f"Conectado à tabela DynamoDB: {DYNAMODB_TABLE} (endpoint: {AWS_ENDPOINT_URL or 'AWS real'})")
